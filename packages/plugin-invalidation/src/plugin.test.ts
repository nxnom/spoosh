...
describe("invalidationPlugin", () => {
  ...
  describe("stripTagPrefix from context", () => {
    it("should strip prefix from context tags in 'all' mode", () => {
      const plugin = invalidationPlugin();
      const stateManager = createStateManager();
      const eventEmitter = createEventEmitter();
      ...
      const context = createMockContext({
        stateManager,
        eventEmitter,
        tags: ["api/v1/users", "api/v1/users/1"],
        stripTagPrefix: "api/v1",
      });
    ...
    it("should strip prefix from self path in 'self' mode", () => {
      const plugin = invalidationPlugin({ defaultMode: "self" });
      const stateManager = createStateManager();
      const eventEmitter = createEventEmitter();
    ...
    it("should strip prefix from explicit tags in array", () => {
      const plugin = invalidationPlugin({ defaultMode: "none" });
      const stateManager = createStateManager();
      const eventEmitter = createEventEmitter();
      ...
      const context = createMockContext({
        ...
        stripTagPrefix: "api/v1",
        pluginOptions: {
          invalidate: ["api/v1/posts", "api/v1/comments"],
        },
      });
      ...
      expect(invalidateHandler).toHaveBeenCalledWith(["posts", "comments"]);
    });

    it("should not strip if path doesn't start with prefix", () => {
      const plugin = invalidationPlugin();
      const stateManager = createStateManager();
      const eventEmitter = createEventEmitter();
    ...
    it("should not accidentally strip 'apian' when prefix is 'api'", () => {
      const plugin = invalidationPlugin();
      const stateManager = createStateManager();
      const eventEmitter = createEventEmitter();
      ...
      expect(invalidateHandler).toHaveBeenCalledWith(["apian/bees"]);
    });

    it("should combine stripTagPrefix with mode + explicit tags", () => {
      const plugin = invalidationPlugin({ defaultMode: "none" });
      const stateManager = createStateManager();
      const eventEmitter = createEventEmitter();
      ...
      expect(calledTags).toContain("users");
      expect(calledTags).toContain("users/1");
      expect(calledTags).not.toContain("api/v1/posts");
      expect(calledTags).not.toContain("api/v1/users");
    });
  });
});
