...
| `autoStart`   | `boolean` | `true`  | Whether to start processing immediately on trigger |
| + plugin opts | -         | -       | Hook-level options from plugins                    |

```typescript
const queue = useQueue((api) => api("uploads").POST(), {
  concurrency: 5,
  autoStart: true, // set to false for deferred start
...
| ----------- | ------------------------------- | ------------------------------------------------------ |
| `trigger`   | `(input?) => Promise<Response>` | Add item to queue, returns when complete               |
...
| `remove`    | `(id?) => void`                 | Remove task by ID, or all finished                     |
| `clear`     | `() => void`                    | Abort all and clear entire queue                       |
| `start`     | `() => void`                    | Start processing (only needed when `autoStart: false`) |
| `isStarted` | `boolean`                       | Whether queue processing has started                   |
...
```typescript
const { tasks, remove } = useQueue((api) => api("uploads").POST());
...
Use `autoStart: false` to queue items without immediately processing them. This is useful when you want to collect all items first, then start processing as a batch:

```typescript
function BatchUploader() {
  const { trigger, tasks, stats, start, isStarted, clear } = useQueue(
    (api) => api("uploads").POST(),
    { autoStart: false, concurrency: 3 }
  );
  ...
  return (
    <div>
      ...
      <p>Queued: {stats.pending} files</p>

      {!isStarted ? (
        <button onClick={start} disabled={stats.total === 0}>
          Start Upload ({stats.total} files)
        </button>
      ) : (
        <div>
          <p>Progress: {stats.percentage}%</p>
...
Once `start()` is called, any new `trigger()` calls will execute immediately (the queue behaves normally).
...
function UploadQueue() {
  const { trigger, tasks, stats, abort, retry, remove, clear } = useQueue(
    (api) => api("uploads").POST(),
    { concurrency: 3 }
  );
  ...
  return (
    <div>
      ...
      <div className="actions">
        ...
        <button onClick={() => remove()} disabled={stats.settled === 0}>
          Remove Finished
        </button>
