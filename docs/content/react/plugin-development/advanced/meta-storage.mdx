...
console.log(meta.summary); // { total: 50, categories: {...}, hasMore: false }
```
...
console.log(meta.transformedData); // { total: 10, titles: [...] }
```
...
console.log(meta.summary.categories); // ✅ Type-safe
```
...
const user = meta.indexed[userId];
```
...
For more on afterResponse, see [After Response](/docs/react/plugin-development/advanced/after-response).

For plugin-to-plugin communication, see [Plugin Communication](/docs/react/plugin-development/advanced/architecture#plugin-communication).
