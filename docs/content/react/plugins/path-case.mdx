...
```typescript
// With pathCasePlugin - clean camelCase paths
...
const { data } = useRead((api) =>
  api("blogPosts/:postId/comments").GET({ params: { postId } })
);
...
useRead((api) => api("legacyEndpoint").GET(), {
  pathCase: { targetCase: "snake" },
});

// Add exclusions for this request
useWrite((api) => api("v2/users").POST, {
  pathCase: { exclude: ["v2"] },
});
```
