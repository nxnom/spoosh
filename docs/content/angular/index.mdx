...
const user = injectRead((api) => api.users(123).$get());
// → tags: ["users", "users/123"]

const userPosts = injectRead((api) => api.users(123).posts.$get());
...
// - users/123
// - users/123/posts
await createPost.trigger({ body: newPost });
```
...
const userPosts = injectRead((api) =>
  api.users(123).posts.$get({ query: { page: 1 } })
);
```
