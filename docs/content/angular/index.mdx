---
title: Introduction
description: A type-safe API client with automatic cache invalidation and zero boilerplate for Angular
---

**Spoosh** is a type-safe API client with automatic cache management for Angular. Define your API schema once, then get type-safe requests with automatic cache invalidation based on URL hierarchy.
...
```typescript
// Cache tags are automatically generated from the path:
const users = injectRead((api) => api("users").GET());
// → tags: ["users"]

const user = injectRead((api) => api("users/:id").GET({ params: { id: 123 } }));
// → tags: ["users", "users/123"]

const userPosts = injectRead((api) =>
  api("users/:userId/posts").GET({ params: { userId: 123 } })
);
...
```typescript
const createPost = injectWrite((api) => api("users/:userId/posts").POST);

// This mutation automatically invalidates:
// - users
// - users/123
// - users/123/posts
await createPost.trigger({ params: { userId: 123 }, body: newPost });
```

### Type-Safe Routing with Path-Based API

Define your API schema once, then use a path-based API that gives you autocomplete for every endpoint, parameter, and response.

```typescript
// Fully typed with path-based syntax
const user = injectRead((api) => api("users/:id").GET({ params: { id: 123 } }));
const userPosts = injectRead((api) =>
...
| `@spoosh/plugin-path-case`        | Write camelCase, send kebab-case to API                |
| `@spoosh/plugin-debug`            | Debug logging                                          |

## Next Steps

<Cards>
  <Card title="Getting Started" href="/angular/docs/getting-started" />
  <Card
    title="Installation"
    href="/angular/docs/getting-started/installation"
  />
</Cards>
