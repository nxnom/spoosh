...
For more on storing user-facing data, see [Meta Storage](/docs/angular/plugin-development/advanced/meta-storage).
...
## Real-World Example: Debug Plugin

The official debug plugin uses `afterResponse` to log all requests:

```typescript
function debugPlugin(): SpooshPlugin {
  return {
    name: "spoosh:debug",
...
For more on middleware patterns, see [Middleware Patterns](/docs/angular/plugin-development/advanced/patterns).
