...
`injectInfiniteRead` works with plugins like cache and deduplication. Each page request has its own cache key, and the merged list is derived from these cached responses. Deduplication prevents duplicate requests when scrolling quickly:
...
Like `injectRead`, `injectInfiniteRead` reacts to changes in the request function. When query parameters change via signals, it automatically fetches with the new parameters:
