...
const writer = injectWrite((api) => api("users/:id").PATCH);
...
await this.postWriter.trigger({ body: newPost, invalidate: "all" });

// Surgical update — only this endpoint's queries refresh
await this.postWriter.trigger({ body: newPost, invalidate: "self" });

// Cross-domain — refresh unrelated queries too
await this.postWriter.trigger({ body: newPost, invalidate: ["all", "dashboard"] });
```
