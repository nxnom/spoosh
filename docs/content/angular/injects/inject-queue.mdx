...
| `isStarted`      | `Signal<boolean>`               | Whether queue processing has started                   |
...
@Component({
  ...
  template: `
    <input
      ...
      [disabled]="queue.isStarted()"
    />

    <p>Queued: {{ queue.stats().pending }} files</p>

    @if (!queue.isStarted()) {
      <button (click)="queue.start()" [disabled]="queue.stats().total === 0">
        Start Upload ({{ queue.stats().total }} files)
      </button>
    } @else {
      <div>
        <p>Progress: {{ queue.stats().percentage }}%</p>
...
export class BatchUploaderComponent {
  ...
  addFiles(event: Event) {
    ...
    for (const file of Array.from(files)) {
      this.queue.trigger({ body: { file, filename: file.name } });
    }
  }
}
```
...
@Component({
  ...
  template: `
    ...
    <div class="actions">
      ...
      <button (click)="queue.remove()" [disabled]="queue.stats().settled === 0">
        Remove Finished
      </button>
