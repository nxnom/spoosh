...
export class UserProfileComponent {
  private user = injectRead((api) => api.users(1).$get());
...
export class CreatePostComponent {
  private createPost = injectWrite((api, data) =>
    api.posts.$post({ body: data })
  );
...
### injectInfiniteRead Abort

```typescript
import { Component, computed } from "@angular/core";
import { injectInfiniteRead } from "./api/client";
...
```typescript
const createUser = injectWrite((api) =>
