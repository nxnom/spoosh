...
export class UserPostsListComponent {
  ...
  private userPosts = injectRead((api) =>
    api.users(this.userId()).posts.$get()
  );
  posts = computed(() => this.userPosts.data());

  private createPost = injectWrite(
    (api) => api.users(this.userId()).posts.$post
  );
  private deletePost = injectWrite(
    (api) => api.users(this.userId()).posts(":postId").$delete
  );

  async handleCreatePost() {
    await this.createPost.trigger({ body: { title: "New Post" } });
  }

  async handleDeletePost(postId: number) {
    await this.deletePost.trigger({ params: { postId } });
  }
}
```
...
See the [Hono Integration](/angular/docs/integrations/hono) guide for details.
...
See the [Elysia Integration](/angular/docs/integrations/elysia) guide for details.
