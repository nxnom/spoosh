...
The cache plugin runs with **priority -10**, meaning it executes early in the middleware chain to check the cache before other plugins (like retry, debug) run.
...
const { injectRead, clearCache } = create(spoosh);
