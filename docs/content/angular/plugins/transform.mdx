...
The Transform plugin enables per-request transformation of query, body, formData, urlEncoded, and response data. All transforms are defined at the inject function level for full type inference.
...
```typescript
const uploadFile = injectWrite((api) => api.files.$post);
...
```typescript
const getToken = injectWrite((api) => api.auth.token.$post);
