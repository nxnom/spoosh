...
Install only the plugins you need. See the [Plugins](/docs/plugins) section for details on each plugin.
...
```package-install
@spoosh/core @spoosh/react @spoosh/plugin-cache @spoosh/plugin-deduplication @spoosh/plugin-invalidation @spoosh/plugin-retry
```
