---
title: OpenAPI
description: Bidirectional conversion between Spoosh schemas and OpenAPI 3.0 specs
---

The `@spoosh/openapi` package provides bidirectional conversion between TypeScript Spoosh schemas and OpenAPI 3.0 specifications.
...
type ApiSchema = {
  posts: {
    /** Retrieve all posts */
    $get: EndpointWithQuery<Post[], { userId?: number }, { error: string }>;
...
The import feature automatically detects the appropriate Spoosh endpoint type based on the OpenAPI operation:

| OpenAPI Pattern                    | Spoosh Type                                      | Example                     |
| ---------------------------------- | ------------------------------------------------ | --------------------------- |
| Query parameters                   | `EndpointWithQuery<TData, TQuery, TError>`       | GET with `?page=1&limit=10` |
| `multipart/form-data` request body | `EndpointWithFormData<TData, TFormData, TError>` | POST with file upload       |
| `application/json` request body    | `Endpoint<TData, TBody, TError>`                 | POST/PUT with JSON          |
...
$post: Endpoint<Data, Body, { system_message?: string }>;

// If no error responses have schemas, no error type parameter is added
$get: Data;
```
...
$post: Endpoint<Data, Body, { error?: string } | { system_message?: string }>;
```
...
```typescript
$post: EndpointWithFormData<Response, { file: File; title: string }>;
```
