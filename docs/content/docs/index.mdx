---
title: Introduction
description: A type-safe API client framework with a powerful plugin system for React
---

**Spoosh** is a lightweight, type-safe API client framework for React applications. It provides a clean, intuitive way to define API schemas and make HTTP requests with full TypeScript support.
...
- **React Hooks** - `useRead`, `useWrite`, and `useInfiniteRead` for seamless data fetching
- **Framework Adapters** - First-class support for Hono with automatic type inference
...
| `@spoosh/react` | React hooks (`useRead`, `useWrite`, `useInfiniteRead`) |
